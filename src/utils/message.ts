...
export function getMessageText(message: proto.IWebMessageInfo): string | null {
  ...
  const pickTextFromRecord = (record: Record<string, unknown> | null): string | null => {
    ...
    return (
      ...
      pickTextFromRecord(record.body as Record<string, unknown> | null) ??
      pickTextFromRecord(record.nativeFlowResponseMessage as Record<string, unknown> | null)
    )
  }
  ...
  const eventResponseToText = (
    value: proto.Message.EventResponseMessage.EventResponseType | null | undefined
  ): string | null => {
    switch (value) {
  ...
  const eventResponse = (normalized as unknown as { eventResponseMessage?: proto.Message.IEventResponseMessage })
  ...
  switch (contentType) {
    ...
    case 'extendedTextMessage':
      return pickText(
        ...
        normalized.extendedTextMessage?.description,
        normalized.extendedTextMessage?.title,
        normalized.extendedTextMessage?.matchedText
      )
    case 'imageMessage':
    ...
    case 'buttonsMessage':
      return pickText(
        normalized.buttonsMessage?.contentText,
        normalized.buttonsMessage?.footerText,
        normalized.buttonsMessage?.text
      )
    ...
    case 'listMessage':
      return pickText(
        normalized.listMessage?.title,
        normalized.listMessage?.description,
        normalized.listMessage?.buttonText
      )
    case 'listResponseMessage':
      return pickText(
        normalized.listResponseMessage?.title,
        normalized.listResponseMessage?.description,
    ...
    case 'pollCreationMessageV5':
      return pickText(
        (
          normalized[contentType] as proto.Message.IPollCreationMessage | null | undefined
        )?.name
      )
    ...
    case 'contactsArrayMessage':
      return pickText(
        normalized.contactsArrayMessage?.displayName,
        normalized.contactsArrayMessage?.contacts?.[0]?.displayName
      )
    case 'locationMessage':
      return pickText(
        normalized.locationMessage?.comment,
        normalized.locationMessage?.name,
        normalized.locationMessage?.address,
        normalized.locationMessage?.url
      )
    case 'liveLocationMessage':
      return pickText(normalized.liveLocationMessage?.caption)
    case 'groupInviteMessage':
      return pickText(
        normalized.groupInviteMessage?.caption,
        normalized.groupInviteMessage?.groupName
      )
    case 'eventMessage':
      return pickText(
        normalized.eventMessage?.name,
        normalized.eventMessage?.description,
        normalized.eventMessage?.joinLink
      )
